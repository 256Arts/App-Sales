...
- `AIUsage` / `AIUsageSignIn` / `AIUsageAPI` / `AIAssistants` — the AI coding assistants' rate limits (Claude and Codex), shown by `AIUsageSection` (main app only). Neither vendor offers a web sign-in an app can drive, so `AIUsageSignIn.read(_:for:)` takes a copy of one the CLI issued: a pasted `claude setup-token` token, `~/.claude/.credentials.json` / the `Claude Code-credentials` Keychain JSON, or `~/.codex/auth.json`. `AIAssistants.shared` is the `@Observable` store, in the synchronizable Keychain (key `ai-assistants`) so a sign-in taken on the Mac reaches phone, watch, and widgets. `AIUsageAPI` reads `api.anthropic.com/api/oauth/usage` and `chatgpt.com/backend-api/wham/usage` — undocumented endpoints the vendors' own CLIs call, so every field is parsed defensively — and refreshes tokens against `platform.claude.com` / `auth.openai.com`. **A refresh token used twice gets its family revoked, which would sign the user's terminal out too**, so `AIAssistants` allows only one fetch per assistant in flight. On macOS the sign-in sheet can also read the credentials file directly, via a user-selected-file grant (`com.apple.security.files.user-selected.read-only`) — the sandbox blocks every other route to it. `AIUsageCache` (App Group, `ai-usage-cache.json`) holds the last reading per assistant so the app, both widget extensions, and the menu bar extra share one fetch rather than each spending their own; `AIUsageDisplay` is the shared reading of the used-vs-left and relative-vs-absolute preferences (App-Group `UserDefaults`), and `AIUsageBar` / `AIUsageColumn` (in `Views/`, compiled into every target) are the views all four surfaces draw.
...
- `App Sales/Views/AIUsageMenuBar.swift` — the Mac's `MenuBarExtra` (a `.window`-style scene in `AppSalesApp`, inserted only while `aiUsageMenuBarExtra` is set) and `LoginItem`, the `SMAppService.mainApp` wrapper behind "Open at Login". macOS-only, and listed in the widget and watch membership exceptions because the macOS widget extension would otherwise compile it without `AIUsageOptions`.
